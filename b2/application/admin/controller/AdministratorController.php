...
class AdministratorController extends AdminAuth
{
    ...
    public function add()
    {
        $model_url = SITE_URL.'/admin/administrator/';
        ...
        if ($id = $user->validate(true)->insertGetId($data)) {
            return $this->success('用户添加成功',$model_url.$id);
        } else {
    ...
    public function update($id)
    {
        ...
        if ($user->update($data)) {
            return $this->success('管理员信息更新成功',$model_url.$id);
        } else {
    ...
    public function upload(){
	    ...
	    if($file){
	        if (true !== $this->validate(['avatar' => $file], ['avatar' => 'image'])) {
	            ...
	            $image->save($data['upload_path'] .'/'. $saveName);
    ...
    public function delete_image($id){
    	...
    	$user = Administrator::get($id);
    	if (file_exists($data['upload_path'] .'/'. $user->avatar)) {
            @unlink($data['upload_path'] .'/'. $user->avatar);
        }

        $source_image = str_replace('_thumb', '', $user->avatar);
        if (file_exists($data['upload_path'] .'/'. $source_image)) {
            @unlink($data['upload_path'] .'/'. $source_image);
        }

        $data['id'] = $id;
        $data['avatar'] = '';
        if ($user->update($data)) {
        	return $this->success('图像删除成功',$data['module_url'].$id);
        }else{
        	return $user->getError();
        }
